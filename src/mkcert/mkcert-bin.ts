...
import type { BaseSource } from './srouces';
import { createSource } from './srouces';
...
class MkcertBin extends Bin {
  ...
  private readonly source: BaseSource;

  constructor(mkcertBinPath: string, version = 'latest', source = 'github') {
    super();
  ...
  async getMkcertBin(): Promise<string> {
    ...
    if (files.length > 0) {
      ...
      if (this.version) {
        const satisfy = versions.find(
          (d) => d.version!.version === this.version,
        );
        if (satisfy) {
    ...
    const mkcert = await this.source.getRelease(
      this.version,
      this.mkcertTargetName,
    );

    if (!mkcert) {
      throw new Error(
        `Failed to download mkcert binary for version '${this.version}' with target '${this.mkcertTargetName}'`,
      );
    }
